...
<p align="center">
Веб-сервис, который выявляет сетевые атаки методами ML. В проекте реализуется инференс с аппаратным ускорением на GPU и асинхронная обработка PCAP-дампов сетевого трафика.
</p>
